...
ipcMain$1.handle("store:finished:remove", (_, learnKey, item) => {
  const modules = store.get("module") || [];
  const updated = modules.map((m) => {
    if (m.learn !== learnKey) return m;
...
ipcMain$1.handle("store:finished:clear", () => {
  const modules = store.get("module") || [];
  const updated = modules.map((m) => ({
    ...m,
    finished: []
    // clear only the finished array
  }));
  store.set("module", updated);
  return updated;
});
ipcMain$1.handle("store:quiz:get", () => {
